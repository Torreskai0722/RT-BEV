...
class UniADTrack(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        self.counter = 0
        self.pre_key_feats = []
        self.nusc = NuScenes(version='v1.0-mini', dataroot='/root/v1.0-mini', verbose=True)
    ...
    def bev_to_uv_coor(self, bboxes, labels, scores, img_metas):
        ...
        # Ensure a deterministic camera order (NuScenes typically has 6 cams)
        # cam_order = list(self.mask_roi_name)  # should be 6 items

        cam_images = {}  # cam -> annotated image (BGR)
        mosaic_order = [
            "CAM_FRONT_LEFT", "CAM_FRONT", "CAM_FRONT_RIGHT",
            "CAM_BACK_LEFT",  "CAM_BACK",  "CAM_BACK_RIGHT"
        ]
        # cam_labels = []  # keep labels aligned with cam_order
        ...
        for cam in self.mask_roi_name:
            ...
            image_path = "/root/v1.0-mini/" + sample_data['filename']  # Assuming sample_data['filename'] contains the path to the image
            ...
            if minimal_bbox:
                ...
                self.draw_one_box_inplace(image, minimal_bbox, color=(0, 0, 255), thickness=3)
            else:
                self.camera_bboxes[cam] = [0, 0, 0, 0]
                # print("No valid bounding boxes found for camera {}".format(cam))
                cv2.putText(image, "NO BOX", (12, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 2, cv2.LINE_AA)

            # Draw all boxes on this camera image
            # self.draw_boxes_inplace(image, box_list)
            ...
            # cv2.destroyAllWindows()
            # process GUI events; don't block
            # key = cv2.waitKey(1) & 0xFF   # or 30 for ~33 FPS
        ...
        # file_roi.close()
        # Build mosaic in cam_order
        # images_for_mosaic = [cam_images[c] for c in cam_order]
        images_for_mosaic = []
        # labels_for_mosaic = cam_order  # show camera names on tiles
        for cam in mosaic_order:
            if cam in cam_images:
                images_for_mosaic.append(cam_images[cam])
            else:
                ph = np.zeros((900, 1600, 3), dtype=np.uint8)
                cv2.putText(ph, f"MISSING: {cam}", (30, 80),
                            cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 0, 255), 3, cv2.LINE_AA)
                images_for_mosaic.append(ph)

        mosaic = self.make_mosaic(
            images_for_mosaic,
            layout=(2, 3),           # 2 rows x 3 cols for 6 cams
            tile_size=(640, 360),    # adjust as desired
            pad=10,
            labels=mosaic_order
        )

        out_dir = "/root/rtbev_outputs"
        os.makedirs(out_dir, exist_ok=True)

        # Use sample token if available; fall back to something stable
        sample_id = sample.get("token", "sample")
        out_path = os.path.join(out_dir, f"mosaic_{sample_id}.png")

        ok = cv2.imwrite(out_path, mosaic)
        if not ok:
            raise RuntimeError(f"Failed to write mosaic image to {out_path}")

        print("Saved mosaic:", out_path)

    def draw_boxes_inplace(self, image, box_list, color=(0, 0, 255), thickness=2):
        """
        box_list: list of [xmin, ymin, xmax, ymax] (can be float)
        clamps to image bounds to avoid cv2 issues.
        """
        h, w = image.shape[:2]
        for b in box_list:
            x1, y1, x2, y2 = b
    ...
    def draw_one_box_inplace(self, image, bbox, color=(0, 0, 255), thickness=3):
        """
        bbox: [xmin, ymin, xmax, ymax] (float or int)
    ...
    def make_mosaic(self, images, layout=(2, 3), tile_size=(640, 360), pad=10, labels=None):
        """
        images: list length rows*cols, each BGR uint8
        layout: (rows, cols) e.g., (2,3)
        tile_size: (tile_w, tile_h) final size of each tile
        labels: optional list of strings same length as images
        """
        rows, cols = layout
        ...
        for r in range(rows):
            tiles = []
            for c in range(cols):
                tiles.append(resized[idx])
                idx += 1
            # pad between tiles horizontally
            if pad > 0:
                sep = np.zeros((tile_h, pad, 3), dtype=np.uint8)
                row = tiles[0]
                for t in tiles[1:]:
        ...
        # pad between rows vertically
        if pad > 0:
            sep_h = np.zeros((pad, row_imgs[0].shape[1], 3), dtype=np.uint8)
            mosaic = row_imgs[0]
            for rr in row_imgs[1:]:
                mosaic = np.vstack([mosaic, sep_h, rr])
        else:
