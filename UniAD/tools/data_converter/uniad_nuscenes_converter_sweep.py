...
def _get_can_bus_info(nusc, nusc_can_bus, sample, scene_name):
...
def _fill_trainval_infos(nusc,
    ...
    for scene_index, scene in enumerate(nusc.scene):
        ...
        info = {
            ...
            'lidar2ego_translation': lidar_cs_record['translation'],
            'lidar2ego_rotation': lidar_cs_record['rotation'],
        ...
        for cam in camera_types:
            ...
            while cam_token != '':
                ...
                cam_info, data = obtain_sensor2top(nusc, cam_token, l2e_t, l2e_r_mat, cam)
                cam_info.update(cam_intrinsic=cam_intrinsic)
                can_bus = _get_can_bus_info(nusc, nusc_can_bus, sample, scene_name)
                info['cams'].update({cam: cam_info})
                info['timestamp'] = data["timestamp"]
                info['can_bus'] = can_bus

                locs = np.array([b.center for b in box_list]).reshape(-1, 3)
                dims = np.array([b.wlh for b in box_list]).reshape(-1, 3)
                rots = np.array([b.orientation.yaw_pitch_roll[0]
                                for b in box_list]).reshape(-1, 1)
...
def obtain_sensor2top(nusc,
    ...
    # Correcting rotation
    R = np.linalg.inv(l2e_r_mat) @ l2e_r_s_mat

    # Correcting translation
    l2e_t_s_transformed = e2g_r_s_mat.T @ l2e_t_s
    combined_translation = l2e_t_s_transformed + e2g_t_s
    T = np.linalg.inv(l2e_r_mat) @ combined_translation

    sweep['sensor2lidar_rotation'] = R.T  # Corrected to directly use R.T
